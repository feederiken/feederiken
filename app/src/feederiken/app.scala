...
object FeederikenApp extends App {
  ...
  def performSearch(prefix: Seq[Byte]) = for {
    creationTime <- now
    stream = genCandidates(creationTime).filter(_.getPublicKey.getFingerprint.startsWith(prefix))
    result <- stream.take(1).runHead.someOrFailException <* log.info("Found matching keypair")
  } yield result
  ...
  def interpret(command: Command): RIO[Env, Unit] = command match {
    case command: Bench => for {
  ...
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] = for {
    ...
    code <- CLI.top.parse(args, env) match {
      ...
      case Right(command) =>
        interpret(command).provideCustomLayer(PGP.bouncyCastle ++ Logging.console()).exitCode
    }
  } yield code
}
